...
export async function POST(req: any) {
  ...
  const { payload, signature } = options.copyAndPastedSigningMethod ? copyAndPastedGeneratePayloadAndSignature(input, Boolean(options.googlePayloadGeneration)) : generatePayloadAndSignature(input, options);
